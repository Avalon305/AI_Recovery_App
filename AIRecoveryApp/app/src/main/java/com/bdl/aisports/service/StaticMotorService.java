...
import com.bdl.aisports.constant.StaticMotorConstant;
...
//    private Intent initLocate = new Intent("locate");
    ...
    public class Controler {
        //连测定位（返回值方式）
        public boolean initLocate(int MotorIndex) {
        ...
        //训练前定位（广播方式）
        public void trainLocate(int MotorIndex,int position,int MotorType) {
            Log.d(TAG, "trainLocate: 训练前定位了！！！！！！！！！！");
            Log.d(TAG, "电机为: "+MotorIndex);
            Log.d(TAG, "位置为: "+position);
            if (MotorIndex == 1) {
                StaticMotorUtil_1.type = MotorType;
                trainSet(StaticMotorUtil_1,position);
            } else if (MotorIndex == 2) {
                StaticMotorUtil_2.type = MotorType;
        ...
        if (intent.getIntExtra("index",0) == 1){      //电机1
            switch (intent.getStringExtra("command")){
                case "SETPOSITION":
                    ...
                    StaticMotorUtil_1.type = intent.getIntExtra("type",0);
            ...
            switch (intent.getStringExtra("command")){
                case "SETPOSITION":
                    ...
                    StaticMotorUtil_2.type = intent.getIntExtra("type",0);
